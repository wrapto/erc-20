...
export const shouldBehaveLikeBridge = async () => {
	...
	it("should fail if amount exceeds balance", async () => {
		...
		await expect(wpac.connect(bob).bridge(pacAddr, decimal(110)))
			.to.be.revertedWithCustomError(wpac, "ERC20InsufficientBalance")
			.withArgs(bob.address, decimal(100), decimal(110))
		// Bob has 100 tokens, trying to bridge 101 should fail
		await expect(wpac.connect(bob).bridge(pacAddr, decimal(101)))
			.to.be.revertedWithCustomError(wpac, "ERC20InsufficientBalance")
			.withArgs(bob.address, decimal(100), decimal(101))
