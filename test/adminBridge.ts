...
export const shouldBehaveLikeAdminBridge = async () => {
	...
	it("should fail if amount exceeds balance", async () => {
		// Minter has 20 tokens, trying to bridge 21 should fail
		await expect(wpac.connect(minter).adminBridge(pacAddr, decimal(21))).to.be.revertedWithCustomError(wpac, "ERC20InsufficientBalance")
			.withArgs(minter.address, decimal(20), decimal(21))
		...
		await expect(wpac.connect(minter).adminBridge(pacAddr, decimal(13))).to.be.revertedWithCustomError(wpac, "ERC20InsufficientBalance")
			.withArgs(minter.address, decimal(12), decimal(13))
	})
